...
const sendHandler = () => {
  ...
  nextTick(() => {
    ...
    tasks.value.forEach((task) => {
      promises.push(
        sendToIntegration(task.desc, task.date, formatDuration(task.duration), task.external_id, task.ids)
          .then(() => {
    ...
    <q-card>
      ...
      <q-card-section class="row q-gutter-md justify-end">
        <q-btn
          ...
          >Send to {{ (getIntegration(settings.integration) as Option).label }}</q-btn
        >
