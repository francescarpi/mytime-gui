...
const SyncModal = (props: SyncProps) => {
  const { opened, tasks, success, updateTaskExtraParam, setDisableSend } = props

  const settingContext = useContext(SettingsContext)
  const { activities, projectActivities, loadRedmineProjectActivities } = useRedmine()
  const [loadingActivities, setLoadingActivities] = useState<boolean>(false)

  useEffect(() => {
    setTimeout(() => {
      // Load the project activities for each task
      if (opened && tasks.length && !loadingActivities && Object.keys(projectActivities).length === 0) {
        setDisableSend(true)
  ...
  useEffect(() => {
    ...
    const defaultActivityName = activities.find(
      (act) => act.id.toString() === settingContext.setting?.integration_config['default_activity'],
    )?.name

    if (defaultActivityName) {
      tasks.forEach((task) => {
        if (!task.extra_param && projectActivities[task.external_id]?.activities) {
  ...
  }, [projectActivities, tasks, activities, settingContext, updateTaskExtraParam])
    ...
    return (
      <TableCell>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          />
          {projectActivities[task.external_id]?.loading && <CircularProgress size={20} sx={{ ml: 2 }} />}
          {projectActivities[task.external_id]?.error && (
